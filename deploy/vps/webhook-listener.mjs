...
const server = createServer(async (request, response) => {
  try {
    ...
    sendJson(response, 202, { ok: true, accepted: true, ref: payload.ref, sha: payload.sha, services: payload.services });
...
server.listen(port, host, () => {
  console.log(`Deploy listener ready on http://${host}:${port}`);
});
