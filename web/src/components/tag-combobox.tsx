...
export const TagCombobox = forwardRef<TagComboboxHandle, {
...
}>(function TagCombobox({ selected, onChange }, ref) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/tags-proxy?${params.toString()}`, { cache: "no-store" });
        if (!res.ok) return;
  ...
  const createTagAndAdd = async (name: string) => {
    ...
    try {
      const res = await authorizedFetch("/api/tags-proxy", {
        method: "POST",
