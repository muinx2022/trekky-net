...
export function PostActions({ targetType, targetDocumentId }: ActionProps) {
  const { isLoggedIn, jwt, openLoginModal, authorizedFetch } = useAuth();
  const [liked, setLiked] = useState(false);
  const [likes, setLikes] = useState(0);
  const [followed, setFollowed] = useState(false);
  const [follows, setFollows] = useState(0);
  const [copied, setCopied] = useState(false);
  const [pendingAction, setPendingAction] = useState<"like" | "follow" | null>(null);
  ...
  useEffect(() => {
    ...
    authorizedFetch(
      ...
      .catch(() => {/* ignore */});
  }, [isLoggedIn, jwt, targetType, targetDocumentId, authorizedFetch]);
  ...
  const toggleInteraction = async (actionType: "like" | "follow") => {
    ...
    try {
      ...
      const active: boolean = !!data?.toggled;
      ...
      setPendingAction((current) => current === actionType ? null : current);
    }
  };
  ...
  return (
    <div className="flex flex-wrap gap-3 border-t border-b border-zinc-200 dark:border-zinc-800 py-4 mt-8">
      ...
      <button
        ...
        } ${pendingAction === "like" ? "opacity-70 cursor-not-allowed" : ""}`}
      >
      ...
      <button
        ...
        } ${pendingAction === "follow" ? "opacity-70 cursor-not-allowed" : ""}`}
      >
