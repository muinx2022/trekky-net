...
async function getUploadErrorMessage(response: Response, fallback: string): Promise<string> {
  const payload = (await response.json().catch(() => null)) as
    | Array<{ message?: string }>
    | { error?: string; message?: string; detail?: string; error_description?: string }
    | null;
  if (Array.isArray(payload)) return fallback;
  return payload?.error || payload?.message || payload?.detail || payload?.error_description || fallback;
}
...
export function PostForm({ mode, documentId }: PostFormProps) {
  ...
  const { authorizedFetch, openLoginModal } = useAuth();
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const payload = (await res.json().catch(() => ([]))) as { results?: DjangoCategoryItem[] } | DjangoCategoryItem[];
        const rows = Array.isArray(payload) ? payload : (payload.results ?? []);
        const byId = new Map<number, CategoryItem>();
        const byDocumentId = new Map<string, CategoryItem>();
        for (const row of rows) {
          if (!row.document_id || !row.name) continue;
          const item = {
            id: row.id ?? 0,
          ...
          byDocumentId.set(row.document_id, item);
        }
        const normalized = rows
          .map((row) => {
            if (!row.document_id || !row.name) return null;
            const parentItem =
              typeof row.parent === "number"
                ? byId.get(row.parent)
                : typeof row.parent === "string"
                  ? byDocumentId.get(row.parent)
                : null;
            return {
              id: row.id ?? 0,
              documentId: row.document_id,
              name: row.name,
              sortOrder: row.sort_order ?? 0,
              parent: parentItem ? { id: parentItem.id, documentId: parentItem.documentId } : null,
            } satisfies CategoryItem;
          })
          .filter((item) => item !== null);
        const categoryItems = normalized as CategoryItem[];
        categoryItems
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoadingPost(true);
      try {
        const res = await authorizedFetch(`/api/my-posts-proxy?documentId=${encodeURIComponent(documentId)}`, {
          cache: "no-store",
        });
    ...
    void load();
    return () => { active = false; };
  }, [mode, documentId, authorizedFetch]);
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (newMediaFiles.length > 0) {
        ...
        const uploadRes = await authorizedFetch("/api/upload-proxy", {
          method: "POST",
          body: formData,
        });
        ...
        if (!uploadRes.ok) {
          ...
          throw new Error(await getUploadErrorMessage(uploadRes, "Tải media lên thất bại"));
        }
        setUploadingMedia(false);
      ...
      if (mode === "create") {
        const res = await authorizedFetch("/api/my-posts-proxy", {
          method: "POST",
      ...
      {activeTab === "content" && (
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <TagCombobox ref={tagComboboxRef} selected={selectedTags} onChange={setSelectedTags} />
          </div>
