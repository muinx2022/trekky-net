<template>
  <header class="sticky top-0 z-50 w-full border-b border-gray-200 bg-white shadow-md">
    <div class="container mx-auto flex h-16 items-center justify-between gap-4 px-4">
      <div class="flex shrink-0 items-center gap-2 pl-1 sm:pl-0">
        <button type="button" class="flex items-center gap-2 md:hidden" aria-label="Mở menu" @click="drawer.openDrawer('right')">
      ...
      <div class="flex items-center gap-1 md:gap-2">
        <button class="flex h-10 w-10 items-center justify-center rounded-full text-gray-600 hover:bg-gray-100 md:hidden" aria-label="Mở tìm kiếm" @click="searchOpen = !searchOpen">
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        <template v-else-if="auth.isLoggedIn.value && auth.user.value">
          ...
          <NuxtLink to="/my-posts/new" class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-500 text-white sm:hidden" aria-label="Tạo bài">+</NuxtLink>
          ...
          <div ref="menuRef" class="relative">
            ...
            <div v-if="menuOpen" class="absolute right-0 mt-2 w-56 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl">
              ...
              <NuxtLink to="/my-posts/new" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50" @click="menuOpen = false">Tạo bài viết</NuxtLink>
              <NuxtLink to="/my-posts" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50" @click="menuOpen = false">Bài viết của tôi</NuxtLink>
              <NuxtLink to="/profile/edit" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50" @click="menuOpen = false">Sửa hồ sơ</NuxtLink>
              <button type="button" class="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-50" @click="toggleThemeFromMenu">
                <span>Giao diện</span>
                <span class="inline-flex items-center gap-2 text-xs text-gray-500">
                  <svg v-if="theme.isDark.value" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div class="border-t border-gray-100">
                <button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-red-600 hover:bg-red-50" @click="logout">Đăng xuất</button>
              </div>
            </div>
          </div>
        </template>
